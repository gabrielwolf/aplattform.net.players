...
export default class OmnitonePlayer {
  ...
  async load() {
    if (this._order === 1) {
      const results = await Omnitone.createBufferList(this._audioContext, [this._src]);
  ...
  play(from: number) {
    ...
    this._currentBufferSource.onended = () => {
      ...
      if (Math.abs(this._durationInSeconds - this.elapsedTimeInSeconds) < lastChanceToStopBeforeEndOfSongInSeconds) {
        clearInterval(this._calcElapsedHandler);
        this._playedFromPosition = .0;
        this._elapsedTimeInMilliSeconds = 0;
